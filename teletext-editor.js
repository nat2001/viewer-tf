		...
		if ( tg[cury][curx] == 0 ) {
			if ( code >= 32 && code <= 127 ) { // and this is a simple text character...
				...
				if ( nd[cury][curx] > 0 && fs[cury] == 1 && cury < 24 ) {
					editor_this.render(curx, cury+1, 1, 1); }
...
var cursor_right = function() {
	...
	if ( curx_opposite == -1 || cury_opposite == -1 ) {
		...
		if ( split == 0 ) { editor_this.render(curx-1, cury, 2, 1, 1); }
		if ( split == 1 ) { 
			editor_this.render(old_curx, old_cury, 1, 1);
			editor_this.render(curx, cury, 1, 1);
		} 
	...
	if ( curx_opposite != -1 && cury_opposite != -1 ) {
		...
		editor_this.render(curx-1, cury, 2, 1, 1);
		editor_this.render(x_to_render, Math.min(cury, cury_opposite),
...
var cursor_left_for_hebrew = function() {
	...
	if ( curx_opposite == -1 || cury_opposite == -1 ) {
		...
		if ( cury > 24 ) { cury = 0; } 
		if ( split == 0 ) { editor_this.render(curx, cury, 2, 1, 1); }
		if ( split == 1 ) { 
			editor_this.render(old_curx, old_cury, 1, 1);
			editor_this.render(curx, cury, 1, 1);
		} 
	...
	if ( curx_opposite != -1 && cury_opposite != -1 ) {
		...
		var x_to_render = old_curx;
		if ( curx < curx_opposite ) { x_to_render = curx; } 
		editor_this.render(curx, cury, 2, 1, 1);
		editor_this.render(x_to_render, Math.min(cury, cury_opposite),
		1, Math.abs(cury - cury_opposite) + 1);
	}
}
...
var cursor_up = function() {
	...
	if ( curx_opposite == -1 || cury_opposite == -1 ) {
		...
		if ( split == 0 ) { editor_this.render(curx, cury, 1, 2, 1); }
		if ( split == 1 ) {
			editor_this.render(old_curx, old_cury, 1, 1);
			editor_this.render(curx, cury, 1, 1);
		} 
	...
	if ( curx_opposite != -1 && cury_opposite != -1 ) {
		...
		editor_this.render(curx, cury, 1, 2, 1);
		editor_this.render(Math.min(curx, curx_opposite), y_to_render,
...
var cursor_down = function() {
	...
	if ( curx_opposite == -1 || cury_opposite == -1 ) {
		...
		if ( split == 0 ) { editor_this.render(curx, cury-1, 1, 2, 1); }
		if ( split == 1 ) {
			editor_this.render(old_curx, old_cury, 1, 1);
			editor_this.render(curx, cury, 1, 1);
		} 
	...
	if ( curx_opposite != -1 && cury_opposite != -1 ) {
		...
		var y_to_render = old_cury;
		if ( cury > cury_opposite ) { y_to_render = cury; } 
		editor_this.render(curx, cury-1, 1, 2, 1);
		editor_this.render(Math.min(curx, curx_opposite), y_to_render,
			Math.abs(curx - curx_opposite) + 1, 1);
	}
}
...
var autorender = function(x,y,w,h) {
	...
	for ( var r = y; r < y+h; r++ ) { 
		...
		if ( affectnext > 0 ) { 
			...
			editor_this.render(from, r, to-from, 1);
		} else { 
			editor_this.render(x, r, w, 1);
		}
	...
	if ( affectnext > 0 && y+h < 25 ) {
		editor_this.render(nextfrom, y+h, nextto-nextfrom, 1);
	}
...
editor_this.render = function(x, y, w, h) {
...
var gfx_change = function(x1, y1, x2, y2) {
	...
	while ( px < 40 ) { 
		...
		if ( cc[py][px] > 16 && cc[py][px] <= 23 && tg[py][px] == 0 ) {
			editor_this.render(px, py, 1, 1); break; }
		if ( blackfg != 0 && cc[py][px] == 16 && tg[py][px] == 0 ) {
			editor_this.render(px, py, 1, 1); break; }
		if ( cc[py][px] > 0 && cc[py][px] <= 7 && tg[py][px] == 1 ) {
			editor_this.render(px, py, 1, 1); break; }
...
// Viewer is called with
/*     function init_frames() {            // Create a new viewer:
          var viewer = new Viewer();            // Make it the active viewer so it receives keypresses:
          active_viewer = viewer;            // Initialise the viewer, placing it in the canvas with HTML
            // ID 'frame'.
          viewer.init_frame('frame');            // Set the user page entry offset
          viewer._viewer_setuserpageoffset(3);            // Set the time display offset and format
          viewer._viewer_settimeformat(32, 'h:m/s');            // Load all pages from this html page
